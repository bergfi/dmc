...
void goPromelaModel() {
    ...
    if(settings["model"].asString() == "bakery.6") {
        model = new DMCModelbakery___prom();
    } else if(settings["model"].asString() == "phils.5") {
        model = new DMCModelphils___prom();
    } else {
        std::cout << "Wrong or no model selected: " << settings["model"].asString() << std::endl;
    }
    if(model) {
...
void goSelectStorage() {
    ...
    if(settings["storage"].asString() == "stdmap") {
        goPromelaModel<llmc::storage::StdMap, ModelChecker>();
    } else if(settings["storage"].asString() == "cchm") {
        goPromelaModel<llmc::storage::cchm, ModelChecker>();
    } else if(settings["storage"].asString() == "treedbs_stdmap") {
        goPromelaModel<llmc::storage::TreeDBSStorage<llmc::storage::StdMap>, ModelChecker>();
    } else if(settings["storage"].asString() == "treedbs_cchm") {
        goPromelaModel<llmc::storage::TreeDBSStorage<llmc::storage::cchm>, ModelChecker>();
    } else if(settings["storage"].asString() == "treedbsmod") {
        goPromelaModel<llmc::storage::TreeDBSStorageModified, ModelChecker>();
    } else if(settings["storage"].asString() == "dtree") {
        goPromelaModel<llmc::storage::DTreeStorage<SeparateRootSingleHashSet<HashSet128<RehasherExit, QuadLinear, HashCompareMurmur>, HashSet<RehasherExit, QuadLinear, HashCompareMurmur>>>, ModelChecker>();
