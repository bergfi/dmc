...
//        size_t aff = sched_getcpu();
        ...
        do {
            tprintf("[%zu] getting new state from shared queue \n", tid);
...
//            }
            ...
            tprintf("[%zu] done all in the local queue, need more states\n", tid);
        } while(true);
        tprintf("[%zu] done.\n", tid);
        return nullptr;
    }
...
//        Context ctx(this, this->_m, 0);
    ...
    llmc::storage::StorageInterface::StateID newTransition(VContext<llmc::storage::StorageInterface>* ctx_, StateID const& stateID, llmc::storage::StorageInterface::StateSlot* slots, size_t length, TransitionInfoUnExpanded const& tinfo) override {
        Context *ctx = static_cast<Context *>(ctx_);
        // the type ID could be extracted from stateID...
        auto insertedState = newState(ctx_, _rootTypeID, length, slots);
        ctx->localTransitions++;
